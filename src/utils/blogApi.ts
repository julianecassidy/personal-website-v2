...
const STRAPI_BASE_URL =
  import.meta.env.STRAPI_URL || "http://localhost:1337/api/";
...
class BlogApi {
  ...
  static async getTaggedPosts(tag_id: number): Promise<Array<Post>> {
    ...
    try {
      ...
      const posts = postsData.data.attributes.personal_blogs.data.map(
        (post: any) => {
          const data = {
            id: post.id,
            title: post.attributes.Title,
  ...
  static async getPost(id: number): Promise<Post> {
    ...
    const post = {
      ...
      tags: postData.data.attributes.categories.data.map((category: any) => {
        const tag = {
          name: category.attributes.Tag,
          id: category.id,
        };
